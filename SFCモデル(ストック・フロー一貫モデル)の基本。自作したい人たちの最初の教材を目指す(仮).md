...
####    ５－３，行動方程式の例
<font color="Purple">行動方程式は、パラメータのカリブレーションや推定が可能な形が選ばれる。</font><font color="Green">が、下に例示する行動方程式が、パラメータのカリブレーションや推定が可能な形かどうかは不明。</font>
...
多くのモデルでは四半期・一年などの一定期間ごとにフローを計算し、ストックを更新するが、連続時間のモデルを作ることもできる。解析的に解くときは、連続時間のモデルを使うことになると思う。<font color="Green">解析的なモデルを使う場合、行動方程式は行列とベクトルの積を使って書くことが多いかもしれない（少なくとも自分が読んだ連続時間のモデルの論文は、2023年10月10日時点でサンプル数１だが、そうしていた）。</font>
